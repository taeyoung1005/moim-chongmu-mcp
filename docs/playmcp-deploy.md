...
- `KAKAO_MAP_JS_KEY` (선택) — 중간장소 결과 페이지 `/meet/*`에서 **카카오 지도**를 켠다. 없으면 카드형 정적 결과로 폴백. Kakao Developers **JavaScript 키** + 플랫폼 도메인에 KC Endpoint 도메인 등록 필요.
- 시크릿 `KAKAO_REST_API_KEY` (선택, live일 때만) — 주소→좌표·주변 장소 검색.
