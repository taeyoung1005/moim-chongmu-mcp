...
function fairnessBlock(fairness: readonly MeetPlanFairness[]): string {
  if (fairness.length === 0) return ""
  const max = Math.max(1, ...fairness.map((row) => row.distanceMeters))
  return [
    `<div class="fairness">`,
    ...fairness.map((row, index) => {
      const pct = Math.max(6, Math.round((row.distanceMeters / max) * 100))
      return [
        `<div class="fair-row">`,
        `<span class="fair-badge">${index + 1}</span>`,
        `<span class="fair-label">${escapeHtml(row.originLabel)}</span>`,
        `<span class="fair-track"><span class="fair-fill" style="width:${pct}%"></span></span>`,
        `<span class="fair-dist">${formatMeters(row.distanceMeters)}</span>`,
        `</div>`,
      ].join("")
    }),
    `</div>`,
