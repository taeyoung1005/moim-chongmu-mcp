...
describe("모임좌표 web availability board", () => {
  it("exposes a short board link and accepts slot selection through HTML form", async () => {
    ...
    expect(initialHtml).toContain("availability-vote-grid")
    expect(initialHtml).toContain("셀을 누른 채로 드래그")
    expect(initialHtml).toContain("pointerdown")
    expect(initialHtml).toContain("availability-vote-grid.is-dragging")
    ...
    expect(updatedHtml).toContain('class="slot vote-cell selected"')
